...
export function SelectCryptoDialogContent() {
  const { setScreen, cryptos, setSelectedCrypto } = useProfileDialogContext();
    ...
    >
      ...
      <CurrencyList>
        {cryptos?.map(crypto => (
          ...
          >
            <CurrencyImage src={crypto.logo} alt={crypto.symbol} />
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <CurrencyName>{crypto.name}</CurrencyName>
              <CurrencySymbol>{crypto.label}</CurrencySymbol>
            </div>
          </CurrencyItem>
        ))}
      </CurrencyList>
